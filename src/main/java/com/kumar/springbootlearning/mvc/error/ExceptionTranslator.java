...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    protected ProblemDetailWithCause customizeProblem(ProblemDetailWithCause problem, Throwable err, NativeWebRequest request) {
        ...
        if (problem.getType() == null || problem.getType()
                .equals(URI.create("about:blank"))) problem.setType(getMappedType(err));
    ...
    private String extractTitle(Throwable err, int statusCode) {
        return getCustomizedTitle(err) !=
                null ? getCustomizedTitle(err) : extractTitleForResponseStatus(err, statusCode);
    }
                ...
                .map(f ->
                        new FieldErrorVM(
                                f.getObjectName()
                                        .replaceFirst("DTO$", ""),
                                f.getField(),
                                StringUtils.hasText(f.getDefaultMessage()) ? f.getDefaultMessage() : f.getCode(),
                                f.getCode()
                        )
                )
                .toList();
    ...
    private String extractTitleForResponseStatus(Throwable err, int statusCode) {
        ...
        return specialStatus == null ? HttpStatus.valueOf(statusCode)
                .getReasonPhrase() : specialStatus.reason();
    }

    private String extractURI(NativeWebRequest request) {
        HttpServletRequest nativeRequest = request.getNativeRequest(HttpServletRequest.class);
        return nativeRequest != null ? nativeRequest.getRequestURI() : Strings.EMPTY;
    }

    private HttpStatus toStatus(final Throwable throwable) {
        // Let the ErrorResponse take this responsibility
        if (throwable instanceof ErrorResponse err) return HttpStatus.valueOf(err.getBody()
                .getStatus());
...
//        if (activeProfiles.contains(Profiles.SPRING_PROFILE_PRODUCTION)) {
//            if (err instanceof HttpMessageConversionException) return "Unable to convert http message";
//            if (err instanceof DataAccessException) return "Failure during data access";
//        }
        return err.getCause() != null ? err.getCause()
                .getMessage() : err.getMessage();
    }
